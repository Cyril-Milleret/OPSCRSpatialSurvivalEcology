...
As an example, we create a $36 \times 36$ habitat grid with a resolution of 4 distance units (du) along each dimension. On the habitat, we center a $28 \times 28$ trapping grid with a resolution of 1 du along each dimension.
...
Next, we set up the objects necessary to perform the local evaluation for the live detection using the 'getLocalObjects' function. Special care should be taken when chosing 'dmax' relative to $\sigma$ [@Milleret2019]. Here we use a value $>3*\sigma$ (see below for the $\sigma$ chosen).
...
In this example, we build an OPSCR model assuming a Markovian and bivariate distribution of the movement of individual activity centers across occasions. We use a multi-state demographic submodel where each individual life history is represented by a succession (implemented with custom distribution `dcatState1Alive1Dead`) of up to three discrete state $z_{i,t}$: 1) 'unborn' if the individual has not been recruited in the population; 2) 'alive' if it is alive; 3)'dead' if it is dead. We assumed mortality (transition from state 2 to 3) is a function of density. Full details are available in Milleret et al. 2022. Estimating spatially variable and density-dependent survival using open-population spatial capture-recapture models; Ecology.
...
sigma <- 2/res(r)[1] # Since we scaled the coordinates to the habitat, we also need to rescale sigma
...
plot(phi ~ dens, ylim=c(0,1), type="b", xlab="Density")

```

### 1.5 Create constants, data, and inits objects
...
Using NIMBLE function 'getDependencies', we obtain the list of nodes to be be simulated. Using the 'simulate' function from NIMBLE, we then simulate the activity center (AC) locations ('s'), the state of the individual ('z') and SCR observation data ('y').
...
Here, we build the NIMBLE model again using the simulated 'y' as data. For simplicity, we use the simulated 'z' as initial values. Then we can fit the OPSCR model with the simulated 'y' data set.
...
In an OPSCR model with density-dependent survival, the log-density is used as a covariate for determining location and time-specific survival rates. As expected for such model structures, use of this covariate induces a strong negative correlation between the posterior samples of the intercept and slope of the linear predictor ($phi_0$ and $\beta_{\phi}$, respectively). This particular model is complicated further by the fact that density itself is a non-constant latent variable, making it more difficult to address this posterior correlation and resulting in slow mixing. Here, we demonstrate that by centering the density ('dens'), that is, try to make log(density+1) have a mean of 0, we can reduce the correlation between 'betaPhi' and 'phi0' and improve the mixing. We can't do this exactly, because density is time- and location-dependent, but we can strive for 0-mean on average. From a preliminary run (in our case the one above), we can note the average of log(density+1) over time and space. For the specific data set simulated in this vignette, mean(log(density)+1) = 1.2.

Now we can modify the model to center density:
...
Now we can rewrite the model to center the density ('dens') by replacing the following line in the model code
...
```{r , eval=T, warning = FALSE, message = FALSE, echo=FALSE, results=FALSE}
...
We now build and fit the model with centered density following the same procedure as shown above:
...
The correlation between phi0 and betaPhi also decreased.
